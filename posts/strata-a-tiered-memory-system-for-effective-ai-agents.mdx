---
title: "Strata: A Tiered Memory System for Effective AI Agents"
date: "2026-05-05"
description: "A tiered memory architecture that combines Strata's lifecycle management with agent-db's relational engine for Phase 2 and Phase 3."
---
...
## Phase 1: The Active Shell (High-Resolution Markdown)

Phase 1 is the agent's working memory. This is the "shell" (the essential, immediate context). It is structured entirely in Markdown files organized in a standard directory tree.
...
The LLM processes the text. It turns a 3,000-word planning document for the iStartup Lab into a concise summary; it tags it with `[iStartup, launch_sprint, Zaretzky]`.
...
## Phase 2: The Medium-Term Orbit (agent-db)

Phase 2 is the semantic and relational memory. Once an active project cools down, those condensed "Memory Blocks" move into **agent-db** (a Postgres-backed system built specifically for autonomous agent memory). Instead of a generic vector database, we get a full relational engine with vector search built in.
...
## The Transition: Phase 2 to Phase 3
...
## Phase 3: The Cold Archive + Shadow Index

This is the most critical part of Strata. How do you find a memory if it's no longer in the active database? You use a Shadow Index.
...
So the synthesis is this: Strata provides the lifecycle discipline (the Janitor, the LRU eviction, the Shadow Index), and agent-db provides the storage engine for Phase 2 and Phase 3. Strata is the Janitor that agent-db is missing; agent-db is the Knowledge Graph that Strata's summaries are too simple to capture on their own.
